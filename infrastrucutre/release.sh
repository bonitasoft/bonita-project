...
# $1 isRelease -- True if tag is create and push
commit_and_push() {
  isRelease=$1
...
usage() {
    ...
    echo "    Let's say you are on master branch and you want to create a 7.12.0 release based on this branch"
    echo ""
    echo ""
    echo "    $ $name 7.12.0"
    echo ""
...
# Change version
./mvnw -ntp versions:set -DnewVersion=$RELEASE_VERSION
